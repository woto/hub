...
class Tables::OffersController < ApplicationController
  ...
  def index
    ...
    context = params.slice(:feed_id, :q, :category_id, :only, :explain).tap do |h|
      h[:from] = (@pagination_rule.page - 1) * @pagination_rule.per
      h[:size] = @pagination_rule.per
    end
