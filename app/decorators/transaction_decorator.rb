...
class TransactionDecorator < ApplicationDecorator
  ...
  def debit_amount
    h.capture do
      if h.current_user.staff? || h.current_user.account_ids.include?(object['_source']['debit_id'])
        h.concat(
          h.tag.mark do
  ...
  def credit_amount
    h.capture do
      if h.current_user.staff? || h.current_user.account_ids.include?(object['_source']['credit_id'])
        h.concat(
          h.tag.mark do
