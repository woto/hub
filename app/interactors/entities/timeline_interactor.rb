  ...
  class TimelineInteractor
    ...
    def call
      ...
      cites.each_cons(2) do |prv, cur, _nxt|
        ...
        if prv.images != cur.images
          operation[:images] = {
            add: GlobalHelper.image_hash(cur.images - prv.images),
            remove: GlobalHelper.image_hash(prv.images - cur.images)
          }
        end
    ...
    def cites
      cites = Cite.order(id: :desc)
                  .includes(:images, :lookups, :topics, :mention, user: [:profile])
                  .limit(context.count)
