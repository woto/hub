...
class Feeds::Offers
  ...
  def flush
    ...
    res = client.bulk(index: Elastic::IndexName.offers(context.feed.index_name),
                      body: @offers.map do |offer|
                        { index: { _id: offer[SELF_NAME_KEY]['id'], data: offer } }
                      end)
  ...
  def hashify(child)
    ...
    unless child.elements.empty?
      raise Feeds::Process::ElasticUnexpectedNestingError, child.to_s
    end

    # TODO: is it safe to use CDATA? What about XSS?
    if child.children.reject { |el| el.text? }.size > 1
      raise Feeds::Process::ElasticUnexpectedNestingError, child.to_s
    end
