...
module Import
  class PreprocessInteractor
    include ApplicationInteractor

    def call
      raise Import::ProcessInteractor::UnknownFileType if context.feed.downloaded_file_type.blank?
      ...
      if context.feed.file.zip?
        ...
        if files.size != 1
          ...
          raise Import::ProcessInteractor::UnzipError, 'Wrong number of unpacked files'
        end
    ...
    def raise_error(out, err)
      ...
      raise Import::ProcessInteractor::UnzipError, [out, err].reject(&:blank?).join("\r\n")
    end
  end
end
