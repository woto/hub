...
//   var buttonHTML = '' +
//       '<button type="button" data-trix-attribute="red" class="trix-button trix-button--icon trix-button--icon-bold" data-trix-attribute="bold" data-trix-key="b" title="${lang.bold}" tabindex="-1">${lang.bold}</button>'
...
addEventListener("trix-initialize", function(event) {
  var buttonHTML = '' +
      '<button type="button" data-trix-action="x-modal">zzz</button>'
  event.target.toolbarElement.
    querySelector(".trix-button-group").
    insertAdjacentHTML("beforeend", buttonHTML)
});
...
document.addEventListener("trix-action-invoke", function(event) {
  ...
  if(event.actionName === "x-modal"){
    ...
    Rails.ajax({
      ...
      success: function(a) {
          // alert(a)
          let attachment = new Trix.Attachment(a);
