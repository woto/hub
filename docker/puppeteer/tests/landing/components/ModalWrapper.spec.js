...
describe('ModalWrapper', function () {
  ...
  it('Shows home page after closing "Modal Login"', async() => {
    ...
    expect(await this.page.url()).to.equal(shared.url('ru'));
  ...
  it('Preserves backdrop url while clicking on links', async () => {
    await this.page.goto(shared.url('', 'feeds/zero_my_index_name/offers/login?page=1&q=zero+one'), { waitUntil: 'networkidle0' });
    ...
    expect(await this.page.url()).to.equal(shared.url('', 'feeds/zero_my_index_name/offers/restore?page=1&q=zero+one'));
  });
