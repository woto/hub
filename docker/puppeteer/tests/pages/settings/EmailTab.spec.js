...
describe('EmailTab', function () {
  ...
  it('Shows success confirmation notice for confirmed users', async () => {
    ...
    await this.page.goto(shared.url('ru', 'settings/email'));
  ...
  it('Shows error if e-mail already taken by another user', async () => {
    await axios.get(shared.url('', 'api/v1/staff/cropper/postgres/crop'));
    await axios.get(shared.url('', 'api/v1/staff/seeder/postgres/create_user'));
    await axios.get(shared.url('', 'api/v1/staff/seeder/postgres/create_another_user'));

    await this.page.goto(shared.url('', 'login'));
    await shared.login.call(this, 'another@example.com', '321321');

    await this.page.goto(shared.url('', 'settings/email'), { waitUntil: 'networkidle0' });
