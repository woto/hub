...
describe('MyAxios', function() {
  ...
  it('renders', async () => {
    ...
    await this.page.waitForSelector('[jid="register-button"]');
    await this.page.click('[jid="register-button"]');
  ...
  it('Adds X-CSRF-Token to headers if it present on a page in meta', async () => {
    ...
    const val2 = await this.page.evaluate(async () => window.MyAxios.defaults.headers.common['X-CSRF-Token']);
  ...
  it('Adds access_token to headers if it present in cookies', async () => {
    ...
    const val2 = await this.page.evaluate(async () => window.MyAxios.defaults.headers.common['Authorization']);
