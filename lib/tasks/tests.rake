...
namespace :hub do
  namespace :tests do
    ...
    task seed: :environment do
      ...
      3.times do |i|
        ...
        10.times do |i|
          ...
          check = nil
          Current.set(responsible: user) do
            check = user.checks.create!(amount: post.amount * rand(0.9..1),
      ...
      2.times do
        OfferCreator.call(
          feed_category: FactoryBot.create(:feed_category),
          name: 'Producto',
    ...
    task graph: :environment do
      ...
      Transaction.group(%i[credit_id credit_label debit_id debit_label])
                 .count.each do |(credit_id, _credit, debit_id, _debit), _count|
        credit_label = Account.find(credit_id).to_label
        key = credit_label
        nodes[key] = g.add_nodes(key) unless nodes.include?(key)
