...
describe SettingsController, type: :system do
  ...
  it 'shows form with empty fields' do
    expect(page).to have_field('new_password', with: '')
  ...
  context 'when new password is too short' do
    it 'shows error' do
      ...
      expect do
        click_on('Save')
        expect(page).to have_text('Password must be at least 6 characters')
