    ...
    context 'when form sends with incorrect values' do
      ...
      it 'shows errors' do
        ...
        within('#new_filter_form') do
          expect(page).to have_css('#filter_form_min ~ .invalid-feedback', text: 'Минимальное значение не может быть пустым')
          expect(page).to have_css('#filter_form_max ~ .invalid-feedback', text: 'Максимальное значение не может быть пустым')
        end
      end
    end
