...
describe MentionPolicy, responsible: :admin do
  ...
  xdescribe '#permitted_attributes' do
    subject { described_class.new(user, nil).permitted_attributes }

    let(:permitted_attributes) do
      [:published_at, :url, :image, :title, :html,
       { related_entities: %i[id is_main], tags: [], advertiser_ext_ids: [], topics_attributes: [] }]
    end
